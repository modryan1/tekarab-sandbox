...
- A Linux environment
- Git
- Network access for cloning public repositories

### Start the API locally
```bash
python3 api.py

If your local environment differs from the current server setup, you may need to install missing Python dependencies first.

Example: start a repo session
curl -sS -X POST http://127.0.0.1:5000/start-repo-session \
  -H 'Content-Type: application/json' \
  -d '{"repo_url":"https://github.com/beary/vite-example"}'
Example: check session status

Replace SESSION_ID with the value returned by start-repo-session.

curl -sS "http://127.0.0.1:5000/session-status?session_id=SESSION_ID"
Expected behavior

Depending on the repository, Tekarab Sandbox may:

return a real web preview when the app becomes reachable
return a terminal-oriented fallback when execution is unclear
defer automatic execution when a safe launch path cannot be inferred
Why this exists

Most repository runners fail in one of two ways:

They reject too many repositories and become useless.
They pretend a repository worked even when it did not truly run.
...
Detect language, repo type, and runnable signals
Infer setup steps and run steps
Apply command safety and rewrite logic when needed
Start an isolated session
Run setup and launch commands
Detect whether a real service becomes reachable
Return the most truthful user experience:
web preview
terminal flow
...
Keep the current support matrix stable
Improve terminal and preview clarity
Add better demo documentation and screenshots
Expand support only after current flows are consistently reliable
Project status
